...
const totalExpenses = computed((): number => {
  return expenses.value
    .filter((e: any) => e.date.startsWith(selectedMonth.value))
...
const handleReset = (): void => {
  console.log("UI Action: Confirm Wipe triggered. Archiving state for:", selectedMonth.value);
  // CONNECTION LOGIC: Save the current totals to history before wiping them
  saveMonthlySnapshot(income.value, savings.value, totalExpenses.value, selectedMonth.value);
...
<template>
  <div class="w-full px-4 sm:px-6 lg:px-12 pt-12 py-6 sm:py-8 sm:pt-20 lg:pt-24 max-w-[1600px] mx-auto">
    <!-- Month Selector -->
    <div class="flex flex-col sm:flex-row justify-between items-center mb-8 gap-4 px-2">
      <div class="text-center sm:text-left">
        <h2 class="text-2xl sm:text-4xl font-black text-gray-900 dark:text-white uppercase tracking-tighter">
          Financial Status
        </h2>
        <p class="text-xs sm:text-sm text-gray-400 font-bold uppercase tracking-widest mt-1">
          Monthly Dashboard Summary
        </p>
      </div>
      <div
        class="flex items-center gap-3 bg-white dark:bg-slate-800 p-2 pl-4 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700">
        <span class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Active Month</span>
        <input type="month" v-model="selectedMonth"
          class="bg-gray-50 dark:bg-slate-900 dark:text-white text-xs font-bold p-2.5 rounded-xl border-none focus:ring-2 focus:ring-indigo-500 cursor-pointer" />
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 lg:gap-8 mb-8 sm:mb-12">
      <div
        ...
        <p
          ...
          K {{ formatCurrency(totalBalance) }}</p>
      </div>
      ...
      <div
        ...
        <p
          ...
          K {{ formatCurrency(totalExpenses) }}</p>
      </div>
    ...
    <Teleport to="body">
      <div v-if="isConfirmingReset"
        ...
        <div
          ...
          <h3 class="text-xl sm:text-2xl font-black text-gray-900 dark:text-white uppercase mb-2">Hard Reset?</h3>
          <p class="text-xs sm:text-sm text-gray-500 dark:text-gray-400 font-medium mb-6 sm:mb-8 leading-relaxed">This
            will wipe your current account, income, savings, and delete all expense entries.</p>
          <div class="flex flex-col gap-2 sm:gap-3">
            <button @click="handleReset"
              class="w-full py-3 sm:py-4 bg-rose-600 text-white font-black rounded-xl sm:rounded-2xl shadow-lg hover:bg-rose-700 transition-all uppercase tracking-widest text-[10px] sm:text-xs cursor-pointer">Confirm
              Wipe</button>
            <button @click="isConfirmingReset = false"
              class="w-full py-3 sm:py-4 bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-300 font-bold rounded-xl sm:rounded-2xl hover:bg-gray-200 transition-all uppercase tracking-widest text-[10px] sm:text-xs cursor-pointer">Cancel</button>
          </div>
        </div>
      </div>
    </Teleport>
    ...
    <div v-if="expenses.length > 0"
      ...
      <div
        ...
        <h3 class="font-black text-gray-400 dark:text-white uppercase text-[10px] sm:text-xs tracking-widest mb-6">
          Expenses Vs Income</h3>
        <ComparisonChart />
      </div>
      <div
        class="w-full bg-white dark:bg-slate-800 p-6 sm:p-8 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700 flex flex-col">
        <h3 class="font-black text-gray-400 dark:text-white uppercase text-[10px] sm:text-xs tracking-widest">Expense
          Categories</h3>
      ...
      <div
        class="w-full bg-white dark:bg-slate-800 p-6 sm:p-8 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700 flex flex-col">
        <h3 class="font-black text-gray-400 dark:text-white uppercase text-[10px] sm:text-xs tracking-widest">Daily
          Spending</h3>
        <div class="pt-5 flex-grow">
          <DailySpending />
        </div>
