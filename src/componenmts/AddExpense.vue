  ...
  <div class="w-full px-4 sm:px-6 lg:px-12 py-4">
    <section
      ...
      <div class="text-center mb-6 sm:mb-8">
        <h2 class="text-2xl sm:text-3xl font-black text-indigo-600 uppercase tracking-tight dark:text-white">
          ADD A NEW EXPENSE
        </h2>
      ...
      <form @submit.prevent="handleSubmit" class="space-y-4 sm:space-y-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
          <div class="flex flex-col gap-1.5 sm:gap-2">
            <label for="desc" class="text-xs sm:text-sm font-bold text-gray-700 dark:text-gray-200">Description</label>
            <input v-model="description" id="desc" type="text" placeholder="e.g. Weekly Groceries"
          ...
          <div class="flex flex-col gap-1.5 sm:gap-2">
            <label class="text-xs sm:text-sm font-bold text-gray-700 dark:text-gray-200">Amount (ZMW)</label>
            <input v-model.number="amount" type="number" step="0.01" placeholder="0.00"
              class="w-full p-3 sm:p-3.5 bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-gray-700 rounded-xl focus:ring-2 focus:ring-indigo-500 outline-none transition-all dark:text-white text-sm sm:text-base"
              required />
          </div>

          <div class="flex flex-col gap-1.5 sm:gap-2">
            <label class="text-xs sm:text-sm font-bold text-gray-700 dark:text-gray-200">Date</label>
          ...
          <div class="flex flex-col gap-1.5 sm:gap-2">
            <label for="category" class="text-xs sm:text-sm font-bold text-gray-700 dark:text-gray-200">Category</label>
...
// Sync date with dashboard month if it changes
watch(selectedMonth, (newMonth) => {
  const currentFullDate = date.value;
  // Only update if the year-month part doesn't match
  if (!currentFullDate.startsWith(newMonth)) {
    date.value = `${newMonth}-01`;
  }
}, { immediate: true });
